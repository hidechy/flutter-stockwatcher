...
class RecordListScreen extends StatefulWidget {
  final String code;
  final String date;
  RecordListScreen({@required this.code, @required this.date});
...
class _RecordListScreenState extends State<RecordListScreen> {
    ...
    for (var i = 0; i < _ym.length; i++) {
      _btnList.add(
        (_ym[i] == _yearmonth)
            ...
            : GestureDetector(
                onTap: () => _goRecordListScreen(date: _ym[i]),
                child: Container(
   ...
   *
   */
  void _goRecordListScreen({String date}) {
   ...
   *
   */
  void _goRecordAllListScreen() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => RecordAllListScreen(
          code: widget.code,
        ),
      ),
    );
